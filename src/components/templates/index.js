import { FavoriteSpecSportChart } from "./makeover-monday"
import { TrustBiases } from "./economist-graphic-detail"
import ChessPlayersDashboard from "./ChessPlayersDashboard"
export { default as Layout } from "./Layout/Layout"
export { default as DataVisualizationGrid } from "./DataVisualizationGrid/DataVisualizationGrid"
export { default as CoronaVirusHungaryDashboard } from "./CoronavirusHungary/CoronavirusHungary"

export { FavoriteSpecSportChart, ChessPlayersDashboard, TrustBiases }
